  ...
  try {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: { temperature: 0.2, thinkingConfig: { thinkingLevel: 'high' }, responseFormat: { text: { mimeType: 'APPLICATION_JSON', schema: response_json_schema } } },
      }),
    });
    if (!response.ok) { const detail = await response.text(); console.error('suggest-build-v2 Gemini error', response.status, detail.slice(0, 1000)); return respond({ ok: false, kind: 'provider_error', error: `Gemini request failed (${response.status})`, details: detail.slice(0, 1000) }); }
